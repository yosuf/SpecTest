...
w :: Coin -> Float 
w (C n) = if n == lighter then 1 - 0.01   {- lighter and heavier are defined at last -}
          else if n == heavier then 1 + 0.01
          else 1

weight :: [Coin] -> Float
weight = sum . (map w)  {- is an infix notation , prefix version would be " map sum W "   -} 
